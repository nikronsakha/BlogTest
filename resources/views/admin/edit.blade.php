            ...
            <div class="mt-8">
                <form enctype="multipart/form-data" method="post" action="{{ route('admin.posts.update',$post['id']) }}"
                    ...
                    <input type="text" name="title" value="{{ $post['title'] }}"
                           class="w-full h-12 border border-gray-800 rounded px-3" placeholder="Название"/>
                    @error('title')
                    <p class="text-red-500">{{ $message }}</p>
                    @enderror
                    <input type="text" name="preview" value="{{ $post['preview'] }}"
                           class="w-full h-12 border border-gray-800 rounded px-3" placeholder="Описание"/>
                    ...
                    <input type="text" name="description" value="{{ $post['description'] }}"
                           class="w-full h-12 border border-gray-800 rounded px-3" placeholder="Текст"/>
